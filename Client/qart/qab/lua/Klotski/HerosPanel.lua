...
function HerosPanel:ctor(mainPage, levelIndex)
	...
	QFramework.QApp.Instance.onUpdate = QFramework.QApp.Instance.onUpdate + self.updateDelegate
...
function HerosPanel:ResetData()
	...
	soundMgr:Play(soundMgr:LoadAudioClip("Sound/gamebegan"),QFramework.QApp.Instance.transform.position)
	...
	if nil ~= self.mainRoleInfo then
		...
		if math.abs(curPosition.x - terminalPos.x) < 2 and math.abs(curPosition.y - terminalPos.y) < 2 then
			...
			if self.stepRecord < preBestStep or preBestStep == 0 then
				...
				soundMgr:Play(soundMgr:LoadAudioClip("Sound/newscore"),QFramework.QApp.Instance.transform.position)

			else 
				soundMgr:Play(soundMgr:LoadAudioClip("Sound/success"),QFramework.QApp.Instance.transform.position)
			end
		end
	end
end
