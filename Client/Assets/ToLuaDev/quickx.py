...
# 这个应该就是构造函数
class OpenSimulatorProjectCommand(sublime_plugin.WindowCommand):

    def run(self):
        view = self.window.active_view()
        completionsFilePath =  sublime.packages_path() + "/ToLuaDev/quickxlib/system_api";
        print(completionsFilePath)
        resaveFiles(completionsFilePath,completionsFilePath)
        # sublime.packages_path
        # launch_simulator(file_name)


# delete files under dir
def resaveFiles(path,root):
    if not os.path.exists(path):
        return
    if os.path.isfile(path):
        try:
            sublime.Window.open_file(path)
            # sublime.save("utf-8")
            f = open(path,"r")
            txt = f.readFile()
            sublime.encode_value(txt)
