...
local EdgeSprite = require "GrayRUn.Game.EdgeSprite"
...
function GameLayer:SetupEdge()
	local stage = self.mCurStage
	if stage == 0 or stage > 100 then
		stage = 1
	end

	self.mCurStage = stage
	local stageInfo = InfoManager.GetStageInfo(self.mCurStage)
	local edgeSprite = EdgeSprite.new()

	edgeSprite:Init(stageInfo)
	edgeSprite:setName("EdgeSprite")
	edgeSprite:addTo(self.transform)
	self:setProperty("edgeSprite",edgeSprite)

	self.mCurStage = self.mCurStage + 1
